...
alpaca["jsonl"] = alpaca.apply(
    lambda x: json.dumps(
        dict(instruction=x["instruction"], input=x["input"], output=x["output"])
    ),
    axis=1,
)
alpaca = alpaca[["jsonl"]]
alpaca.to_csv(
    f"open-alpaca-data-{LC(alpaca)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
...
alpaca_gpt4["jsonl"] = alpaca_gpt4.apply(
    lambda x: json.dumps(
        dict(instruction=x["instruction"], input=x["input"], output=x["output"])
    ),
    axis=1,
...
alpaca_gpt4.to_csv(
    f"open-alpaca-gpt4-data-{LC(alpaca_gpt4)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
...
guanaco.to_csv(
    f"open-guanaco-data-{LC(guanaco)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
...
instructwild["jsonl"] = instructwild.apply(
    lambda x: json.dumps(
        dict(instruction=x["instruction"], input=x["input"], output=x["output"])
    ),
    axis=1,
...
instructwild.to_csv(
    f"open-instructwild-data-{LC(instructwild)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
...
gpteacher["jsonl"] = gpteacher.apply(
    lambda x: json.dumps(
        dict(instruction=x["instruction"], input=x["input"], output=x["output"])
    ),
    axis=1,
...
gpteacher.to_csv(
    f"open-gpteacher-data-{LC(gpteacher)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
...
hc3["chatgpt_output"] = hc3["chatgpt_answers"].map(
    lambda x: "" if len(x) == 0 else x[0]
)
hc3["jsonl1"] = hc3[["instruction", "input", "human_output"]].apply(
    lambda x: json.dumps(
        dict(instruction=x["instruction"], input=x["input"], output=x["human_output"])
    ),
    axis=1,
)
hc3["jsonl2"] = hc3[["instruction", "input", "chatgpt_output"]].apply(
    lambda x: json.dumps(
        dict(instruction=x["instruction"], input=x["input"], output=x["chatgpt_output"])
    ),
    axis=1,
)
...
hc31[["jsonl1"]].to_csv(
    f"open-hc3-human-data-{LC(hc31)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
...
hc32[["jsonl2"]].to_csv(
    f"open-hc3-chatgpt-data-{LC(hc32)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
...
lamini = lamini[["jsonl"]]
lamini.to_csv(
    f"open-lamini-data-{LC(lamini)}K.jsonl",
    sep="\t",
    index=False,
    header=None,
    quoting=3,
