...
def filter_frames(df, tag1_value, tag2_value):
    ...
    filtered_df.drop(columns=['Index', 'User', 'Comment', 'Label']).to_csv('filtered_frames.tsv', sep='\t', index=False)
    return filtered_df
...
def analyze_comments(df):
    ...
    comments_list = [f"{i}: (Image: {image})(Label: {label}; {comment})" for i, (comment, label,image) in enumerate(zip(non_empty_comments['Comment'].tolist(), non_empty_comments['Label'].tolist(), non_empty_comments['start_frame'].tolist()))]
