...
def read_and_join_tsv_files(file_paths, columns_to_keep, column_names, join_key, join_column_names):
    """
    ...
    df = read_and_filter_tsv(file_paths_list[0], columns_to_keep_list, column_names_list)
    print("Column names of the initial DataFrame:", df.columns)
    df2 = read_and_filter_tsv(file_paths_list[1], columns_to_keep_list, column_names_list)
    print("Column names of the 2nd DataFrame:", df2.columns)
    df_merge = df.merge(df2[join_column_names_list], on=join_key, how='left',suffixes=('_0','_1'))
    ...
    for cnt in range(2, len(file_paths_list)):
        ...
        temp_df = read_and_filter_tsv(file_path, columns_to_keep_list, column_names_list)
        df_merge = df_merge.merge(temp_df[join_column_names_list], on=join_key, how='left', suffixes=('',f"_{cnt}"))
        print("Column names after merge:", df_merge.columns)
