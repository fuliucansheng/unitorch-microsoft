...
class WanLoraWrapper:
    ...
    def load_lora(self, lora_path, lora_name=None):
        ...
        print(f"Registered LoRA metadata for: {lora_name} from {lora_path}")
    ...
    def apply_lora(self, lora_name, alpha=1.0):
        if lora_name not in self.lora_metadata:
            print(f"LoRA {lora_name} not found. Please load it first.")

        lora_weights = self._load_lora_file(self.lora_metadata[lora_name]["path"])
        self._apply_lora_weights(lora_weights, alpha)

        print(f"Applied LoRA: {lora_name} with alpha={alpha}")
        del lora_weights
        return self.model

    @torch.no_grad()
    def _apply_lora_weights(self, lora_weights, alpha):
        lora_pairs = {}
        lora_diffs = {}
        ...
        for name in total_key:
            if name not in applied:
                print(f"Warning: LoRA weight not applied for {name}")
                missed += 1

        print(f"Total LoRA weights not applied: {missed}")
    ...
    def remove_lora(self):
        ...
        print(f"LoRA removed, restored {restored_count} weights")
