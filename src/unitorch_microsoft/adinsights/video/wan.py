...
def load_model(ckpt_folder):
    ...
    model_manager.load_models(
        [os.path.join(ckpt_folder, 'models_clip_open-clip-xlm-roberta-large-vit-huge-14.pth')],
        torch_dtype=torch.float32, # Image Encoder is loaded with float32
    )
    model_manager.load_models(
        [
            [
                os.path.join(ckpt_folder, 'diffusion_pytorch_model-00001-of-00007.safetensors'),
                os.path.join(ckpt_folder, 'diffusion_pytorch_model-00002-of-00007.safetensors'),
                os.path.join(ckpt_folder, 'diffusion_pytorch_model-00003-of-00007.safetensors'),
                os.path.join(ckpt_folder, 'diffusion_pytorch_model-00004-of-00007.safetensors'),
                os.path.join(ckpt_folder, 'diffusion_pytorch_model-00005-of-00007.safetensors'),
                os.path.join(ckpt_folder, 'diffusion_pytorch_model-00006-of-00007.safetensors'),
                os.path.join(ckpt_folder, 'diffusion_pytorch_model-00007-of-00007.safetensors'),
            ],
...
def generation(pipe, start_frame, prompt, neg_prompt, cache_dir, infer_step=10):
    ...
    try:
        video = pipe(
            prompt=prompt,
            negative_prompt=neg_prompt+"Bright tones, overexposed, static, blurred details, subtitles, style, works, paintings, images, static, overall gray, worst quality, low quality, JPEG compression residue, ugly, incomplete, extra fingers, poorly drawn hands, poorly drawn faces, deformed, disfigured, misshapen limbs, fused fingers, still picture, messy background, three legs, many people in the background, walking backwards",
            input_image=image,
            height = image.height,
            width = image.width,
        ...
        name = hashlib.md5(start_frame.encode()).hexdigest() + f"_step{infer_step}_10s.mp4"
        print(name)
...
def image2video(
    ...
    for _, row in data.iterrows():
        ...
        video = generation(pipe, _start_frame, _prompt, _neg_prompt, cache_dir, infer_step=10)
        if video != None:
            record = {
                "prompt": _prompt,
