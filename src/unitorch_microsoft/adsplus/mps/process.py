...
class MPSImageProcessor:
    ...
    def classification(self, text: str, condition: str, image: Union[Image.Image, str]):
        """
...
class MPSImagePairProcessor:
    ...
    @classmethod
    @add_default_section_for_init("microsoft/process/mps/pair")
    def from_core_configure(cls, config, **kwargs):
        pass
    ...
    def classification(
        ...
        text_outputs = self._tokenize(text)
        condition_outputs = self._tokenize(condition)
        pixel_outputs0 = self._process_image(image0).squeeze(0)
        pixel_outputs1 = self._process_image(image1).squeeze(0)
        return TensorsInputs(
            {
                "text_inputs": text_outputs,
                "condition_inputs": condition_outputs,
