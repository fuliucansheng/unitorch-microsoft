...
class BletchleyForSLABRanking(GenericModel):
    ...
    def __init__(
        ...
        self.query_projection = nn.Linear(
            ...
            projection_dim,
        )  # query_encoder.projection.weight, query_encoder.projection.bias
...
# 3 text towers ranking model
@register_model("microsoft/adsplus/slab/bletchley/v1/rankingv2")
class BletchleyForSLABRankingV2(GenericModel):
    replace_keys_in_state_dict = {
        "text_encoder.projection": "text_projection",
        "image_encoder.projection": "image_projection",
    }
    ...
    def __init__(
        ...
        output_query_embed: Optional[bool] = False,
        output_text_embed: Optional[bool] = False,
        output_title_embed: Optional[bool] = False,
    ):
        super().__init__()
        query_config = get_bletchley_text_config(config_type, gradient_checkpointing)
        query_config.num_hidden_layers = num_query_layers
        text_config = get_bletchley_text_config(config_type, gradient_checkpointing)
        title_config = get_bletchley_text_config(config_type, gradient_checkpointing)

        self.output_query_embed = output_query_embed
        self.output_text_embed = output_text_embed
        self.output_title_embed = output_title_embed

        self.query_embed_dim = query_config.hidden_size
        self.text_embed_dim = text_config.hidden_size
        self.title_embed_dim = title_config.hidden_size
        ...
        self.title_encoder = BletchleyTextEncoder(
            title_config, add_projection_layer=False
        )
        ...
        self.title_projection = nn.Linear(
            self.title_embed_dim,
            projection_dim,
        )  # title_encoder.projection.weight,  title_encoder.projection.bias
        ...
        self.classifier = nn.Linear(projection_dim * 3, num_classes)
        self.init_weights()
    ...
    @classmethod
    @add_default_section_for_init("microsoft/adsplus/slab/bletchley/v1/rankingv2")
    def from_core_configure(cls, config, **kwargs):
        config.set_default_section("microsoft/adsplus/slab/bletchley/v1/rankingv2")
        config_type = config.getoption("config_type", "0.3B")
        ...
        output_query_embed = config.getoption("output_query_embed", False)
        output_text_embed = config.getoption("output_text_embed", False)
        output_title_embed = config.getoption("output_title_embed", False)
        ...
        inst = cls(
            ...
            output_query_embed=output_query_embed,
            output_text_embed=output_text_embed,
            output_title_embed=output_title_embed,
        )
        pretrained_weight_path = config.getoption("pretrained_weight_path", None)
        if pretrained_weight_path is not None:
            inst.from_pretrained(pretrained_weight_path)
    ...
    def forward(
        ...
        if not self.training and self.output_title_embed:
        ...
        outputs = self.classifier(
            F.relu(torch.cat([query_embeds, text_embeds, title_embeds], axis=1))
        )
