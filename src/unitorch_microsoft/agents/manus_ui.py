...
class ManusAgent(GenericAgent):
    ...
    async def execute(self, instruction: str, **kwargs):
        ...
        if not os.path.exists("/tmp/unitorch_microsoft/agents/manus"):
            os.makedirs("/tmp/unitorch_microsoft/agents/manus")

        workspace = tempfile.mkdtemp(dir="/tmp/unitorch_microsoft/agents/manus")
        ...
        preview_message = Message.system_message(
            content=self._preview_prompt,
        )
...
def cli_main(host: str = "0.0.0.0", port: int = 7050):
    ...
    title = gr.Markdown(
        "# <div style='margin-top:10px; text-align: center'>Unitorch Microsoft Manus Agent</div>"
    )
    ...
    async def _go(ins):
        async for logs, step in manus.execute(ins["text"], files=ins["files"]):
            yield logs, step
            await asyncio.sleep(2)
    ...
    timer.tick(
        fn=manus.get_question,
        outputs=[question],
    )

    slider.change(
        lambda x: manus.get_step_state(x),
        inputs=[slider],
        outputs=[preview],
    )
    ...
    save_button.click(
        lambda: gr.Dataset(samples=manus.save()),
        inputs=[],
        outputs=[examples.dataset],
    )
    delete_button.click(
        lambda: gr.Dataset(
            samples=manus.delete(),
        ),
        inputs=[],
        outputs=[examples.dataset],
    )
    ...
    demo.load(
        lambda: gr.Dataset(samples=manus.records()),
        inputs=[],
        outputs=[examples.dataset],
    )
    ...
    js = ""
    demo.title = "Unitorch Microsoft Manus Agent"
