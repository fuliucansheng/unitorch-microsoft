...
- `POST /microsoft/apps/spaces/picasso/detr/generate1`: Get the bounding box for the input image.
    * Request:
    ...
    ```
    * Response: The response is an image bytes with the bounding box drawn on it.
- `POST /microsoft/apps/spaces/gpt/image/generate`: Get the image generation result for the input prompt.
    * Request: size should be one of "1024x1024", "1536x1024" or "1024x1536".
    ```bash
    curl -X 'POST' \
    '{api_base_url}/microsoft/apps/spaces/gpt/image/generate?prompt=a%20house&size=1024x1024&background=transparent' \
    -H 'accept: application/json' \
    -d ''
    ```
    * Response: The response is an image bytes generated by the model.
- `POST /microsoft/apps/spaces/gpt/image/edit`: Get the image editing result for the input image and prompt.
    * Request: size should be one of "1024x1024", "1536x1024" or "1024x1536".
    ```bash
    curl -X 'POST' \
    '{api_base_url}/microsoft/apps/spaces/gpt/image/edit?prompt=put%20the%20first%20logo%20on%20the%20top%20right%20corner%20of%20the%20second%20image&size=1536x1024&background=transparent' \
    -H 'accept: application/json' \
    -H 'Content-Type: multipart/form-data' \
    ...
    ```
    * Response: The response is an image bytes generated by the model.
- `POST /microsoft/apps/spaces/gpt/chat/generate`: Get the chat completion result for the input prompt, with optional image inputs.
    * Request: `model` overrides the default model; `system` sets the system prompt; `images` accepts one or more image files for vision input.
    ```bash
    curl -X 'POST' \
    '{api_base_url}/microsoft/apps/spaces/gpt/chat/generate?prompt=describe%20the%20image' \
    -H 'accept: application/json' \
    -H 'Content-Type: multipart/form-data' \
    -F 'images=@debug.png;type=image/png'
    ```
    * Response:
    ...
    ``` Get the image generation result for the input prompt.
