...
class ParserScript(GenericScript):
    ...
    def launch(self, **kwargs):
        ...
        for i, _data in enumerate(data):
            ...
            _data["answer"] = _data.results.map(lambda r: r[0]["text"] if len(r) > 0 else "")
            _data["prompt_tokens"] = _data.response.map(lambda r: r["response"]["usage"]["prompt_tokens"])
            _data["completion_tokens"] = _data.response.map(lambda r: r["response"]["usage"]["completion_tokens"])
            _data["total_tokens"] = _data.response.map(lambda r: r["response"]["usage"]["total_tokens"])
