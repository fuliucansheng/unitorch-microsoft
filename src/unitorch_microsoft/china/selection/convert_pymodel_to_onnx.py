...
class OnnxConverter:
    ...
    def export(self, query_input_ids=None, do_optimize=False, do_quantize=True):
        ...
        if do_optimize:
            ...
            optimized_model = optimizer.optimize_model(
                onnx_path, optimization_options=fusion_options
            )
            optimized_model.save_model_to_file(opt_onnx_path)
            onnx_path = opt_onnx_path
        if do_quantize:
            quant_onnx_path = os.path.join(
                self.OutputFolder, "model.optimized.quantized.onnx"
            )
            quantization.quantize_dynamic(onnx_path, quant_onnx_path)
    ...
    def ParityCheck(self, onnx_path, query_input_ids):
        # output from model
        ...
        cosine = np.dot(embedding[0], outputs[1][0].T) / (
            np.linalg.norm(embedding[0]) * np.linalg.norm(outputs[1][0])
        )
    ...
    def ParityCheckPair(
        ...
        cosine1 = np.dot(embedding1, embedding2.T) / (
            np.linalg.norm(embedding1) * np.linalg.norm(embedding2)
        )
        cosine1_2 = np.dot(embedding1, embedding2.T)
        cosine2 = np.dot(outputs1[1], outputs2[1].T) / (
            np.linalg.norm(outputs1[1]) * np.linalg.norm(outputs2[1])
        )
        cosine2_2 = np.dot(outputs1[1], outputs2[1].T)
        cosine3 = np.dot(embedding1, outputs1[1].T) / (
            np.linalg.norm(embedding1) * np.linalg.norm(outputs1[1])
        )
...
class BletchleyForTextPretrainQueryEncoder(nn.Module):
    ...
    def check_load(self, model_state_dict, ckpt_state_dict):
        ...
        print(
            "%f params in model can't find weight"
            % (float(len(non_load_keys)) / len(model_state_dict))
        )
        print(non_load_keys)
        print(check_node)
    ...
    def forward(
        ...
        query_attention_mask = (
            torch.ones([1, query_input_ids.shape[1]]).reshape(1, -1).to(query_input_ids)
        )
...
if __name__ == "__main__":
    ...
    input_ids = torch.tensor(
        [6, 4870, 6, 13525, 6, 28188, 6, 6442, 6, 7006, 6, 14046, 1, 1, 1, 1]
    ).unsqueeze(0)
    print(input_ids.shape)

    # prepare model
    model_file = "/home/xucha/unitorch-microsoft/src/unitorch_microsoft/ckpt/pytorch_model_quant.bin"
    model = BletchleyForTextPretrainQueryEncoder(
        query_config_type="0.15B", weight_path=model_file, projection_dim=64
    )
    model.eval()
