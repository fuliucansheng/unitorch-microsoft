...
from unitorch_microsoft.models.bletchley.pipeline_v3 import BletchleyForMatchingPipeline


@register_webui("core/webui/matching/bletchley")
class BletchleyMatchingWebUI(SimpleWebUI):
    pretrained_names = list(pretrained_bletchley_v3_infos.keys())
    supported_pretrained_names = matched_pretrained_names(
        pretrained_names,
        ".*",
    )
    pretrained_extension_names = list(pretrained_bletchley_v3_extensions_infos.keys())
        ...
        for lora in loras:
            lora.checkpoint.change(
                fn=lambda x: nested_dict_value(
                    pretrained_bletchley_v3_extensions_infos, x, "text"
                ),
...
@register_webui("core/webui/bletchley/v3")
class BletchleyWebUI(SimpleWebUI):
    def __init__(self, config: CoreConfigureParser):
        ...
        super().__init__(config, iname="Bletchley V3", iface=iface)
