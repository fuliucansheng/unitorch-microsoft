    ...
    def forward(self, x, size):
        ...
        if len(x.size()) == 3:
            if self.norm and self.pre_norm:
                x = self.norm(x)
            # x = rearrange(x, "b (h w) c -> b c h w", h=H, w=W) # need einops
            xsz = x.size()
            x = x.permute(0, 2, 1).reshape(xsz[0], xsz[-1], H, W)

        x = self.proj(x)

        b, c, H, W = x.shape
        # x = rearrange(x, "b c h w -> b (h w) c") # need einops
...
class DaViT(nn.Module):
    ...
    def __init__(
        ...
        if self.out_norm and self.out_features is not None:
    ...
    def from_state_dict(self, pretrained_dict, pretrained_layers=[], verbose=True):
        ...
        pretrained_dict = {
            stripped_key(full_key_mappings[k]): v
    ...
    def forward_features(self, x):
        ...
        for conv, block in zip(self.convs, self.blocks):
            ...
            if self.out_features is not None:
                name = f"stage{block_index + 2}"
                if name in self.out_features:
                    x_out = x
                    C = x_out.shape[-1]
                    H, W = input_size
                    if self.out_norm:
                        norm_layer = getattr(self, f"norm{block_index}")
