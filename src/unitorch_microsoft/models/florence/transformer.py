...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, key_padding_mask: torch.Tensor = None):
        self.attention_mask = (
            self.attention_mask.to(dtype=x.dtype, device=x.device)
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.enable_checkpoOptional[int] = enable_checkpoint
        attention_mask = self.build_attention_mask() if autogressive else None
        dpr = [
            x.item() for x in torch.linspace(0, drop_path, layers)
        ]  # stochastic depth decay rule
        self.resblocks = nn.ModuleList(
            [
                ResidualAttentionBlock(width, heads, attention_mask, dpr[i])
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        input_ids = (input_ids * attention_mask).to(torch.long)
        ...
        return x[torch.arange(x.size(0)), input_ids.argmax(dim=-1)]
        # return x[:, 0]
