...
def image2video(
    ...
    def producer():
        ...
        for _, row in data.iterrows():
            ...
            params = { 
                "promptText": _prompt,
                "negativePrompt": _neg_prompt
            }
            image_byte = prepare_image_byte(_start_frame)
            if image_byte == None:
                continue
            image = {
                "image": ("image.jpg", image_byte, "image/jpg")
            }
            try:

                response = send_request_retry(api_key, "https://devapi.pika.art/generate/2.2/i2v", params, image)
                print(response)
                Q.put((response["video_id"],_prompt, _neg_prompt, _index_id, _start_frame, _end_frame))
