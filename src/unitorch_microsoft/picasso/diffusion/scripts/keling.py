...
def send_request_retry(token, api, params, retry_cnt=5):
    ...
    if response['code'] == 1201:
        sleep_time = 1
        retry_cnt = 2
    while ('data' not in response or response['data'] is None or 'task_id' not in response['data'] or response['data']['task_id'] is None) and retry <= retry_cnt:
        retry += 1
    ...
    def producer():
        for _, row in data.iterrows():
            ...
            if neg_prompt_col != None:
                _neg_prompt = row[neg_prompt_col] if row[neg_prompt_col] != np.isnan else ""
            _index_id = ""
            if index_col != None:
            ...
            try:
                response = send_request_retry(api_key, "https://api.klingai.com/v1/images/generations", param)
                print(response)
...
def text2video(
    ...
    def producer():
        for _, row in data.iterrows():
            ...
            try:
                response = send_request_retry(api_key, "https://api.klingai.com/v1/videos/text2video", params)
                print(response)
                Q.put((response['data']['task_id'],_prompt, _neg_prompt, _index_id))
            except:
                pass
    ...
    def producer():
        for _, row in data.iterrows():
            ...
            if start_frame_col != None:
                _start_frame = row[start_frame_col] if not pd.isna(row[start_frame_col]) else None
            if end_frame_col != None:
                _end_frame = row[end_frame_col] if not pd.isna(row[end_frame_col]) else None
            ...
            try:
                response = send_request_retry(api_key, "https://api.klingai.com/v1/videos/image2video", params)
                print(response)
                Q.put((response["data"]["task_id"],_prompt, _neg_prompt, _index_id, _start_frame, _end_frame))
            except:
                pass
