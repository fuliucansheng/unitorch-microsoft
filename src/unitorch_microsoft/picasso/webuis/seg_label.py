...
@register_webui("microsoft/picasso/webui/seg_label")
class SegmentationLabelingWebUI(SimpleWebUI):
    def __init__(
        self,
        config: CoreConfigureParser,
        default_section: str = "microsoft/picasso/webui/seg_label",
        default_name: str = "Picasso Segment Labeling",
        http_url: str = None,
    ):
        self._config = config
        ...
        self.choices1 = config.getoption("choices1", None)
        self.checkbox1 = config.getoption("checkbox1", False)
        self.choices2 = config.getoption("choices2", None)
        ...
        if isinstance(self.choices1, str):
            self.choices1 = re.split(r"[,;]", self.choices1)
            self.choices1 = [c.strip() for c in self.choices1]
        self.choices1 = [c.replace(",", " ").strip() for c in self.choices1]
        if isinstance(self.choices2, str):
            self.choices2 = re.split(r"[,;]", self.choices2)
            self.choices2 = [c.strip() for c in self.choices2]
        self.choices2 = [c.replace(",", " ").strip() for c in self.choices2]
        if isinstance(self.choices3, str):
    ...
    def sample(self, group=None, random_type="Unlabeled"):
        ...
        sampled_data = sampled_data[sampled_data["Index"] != " - "]
