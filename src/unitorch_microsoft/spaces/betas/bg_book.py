...
def call_nano_banana_generate(prompt: str, images: List[Image.Image]) -> Image.Image:
...
def padding_to_the_ratio(image: Image.Image, ratio: float) -> Image.Image:
    ...
    # padded_image = get_recraft_inpainting_image("simple background", new_image, mask)
    return padded_image


class BookBGWebUI(SimpleWebUI):
    _title = "Book Product Enhancement"
    ...
    def __init__(self, config: CoreConfigureParser):
        ...
        output_text2 = create_element(
            "text", "GPT5 Prompt for Background Change", lines=6
        )
        ...
        generate.click(
            ...
            outputs=[output_image1, output_text2, output_image3, output_image4],
            trigger_mode="once",
        )
    ...
    def generate(self, image, ratio):
        prompt1 = "generate a book product with a white background from the provided cover image"
        prompt2 = "design a new attractive non-white background for the product, output the prompt about how to edit to change it into the result. make sure there is no price, brand, link, qr code. Output the prompt in <ans></ans> tag."

        step1 = call_nano_banana_generate(prompt1, [image])
        for _ in range(3):
            step2 = get_gpt5_response(prompt2, images=[step1])
            if re.search(r"<ans>(.*?)</ans>", step2, re.DOTALL):
                step2 = (
                    re.search(r"<ans>(.*?)</ans>", step2, re.DOTALL).group(1).strip()
                )
                break
        step3 = padding_to_the_ratio(step1, ratio)
        step4 = call_nano_banana_generate(step2, [step3])
