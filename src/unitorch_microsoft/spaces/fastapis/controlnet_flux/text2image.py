...
class ControlNetFluxForText2ImageGenerationFastAPIPipeline(GenericStableFluxModel):
    def __init__(
        self,
        config_path: str,
    ...
    @classmethod
    @add_default_section_for_init(
        "microsoft/spaces/fastapi/pipeline/controlnet_flux/text2image"
    )
        ...
        **kwargs,
        ...
        pretrained_controlnet_names = (
            ...
            if pretrained_controlnet_names is not None
                ...
                "pretrained_controlnet_names", "stable-flux-controlnet-dev-union-pro"
            )
        )
    ...
    def __call__(
        ...
        controlnet_conditioning_modes: Optional[List[str]] = [],
...
@register_fastapi("microsoft/spaces/fastapi/controlnet_flux/text2image")
class ControlNetFluxText2ImageGenerationFastAPI(GenericFastAPI):
    def __init__(self, config: CoreConfigureParser):
        self.config = config
        config.set_default_section(
            f"microsoft/spaces/fastapi/controlnet_flux/text2image"
        )
        router = config.getoption(
            "router", "/microsoft/spaces/fastapi/controlnet_flux/text2image"
        )
        self._pipe = None
    ...
    def start(self):
        ...
        self._pipe = (
            ControlNetFluxForText2ImageGenerationFastAPIPipeline.from_core_configure(
                self.config
            )
        )
    ...
    async def generate1(
        ...
        controlnet_guidance_scale: Optional[float] = 2.5,
        controlnet_conditioning_mode: Optional[float] = 0,
        num_timesteps: Optional[int] = 50,
        ...
        controlnet_image_bytes = await controlnet_image.read()
        controlnet_image = Image.open(io.BytesIO(controlnet_image_bytes))
        async with self._lock:
            image = self._pipe(
                text,
                controlnet_images=[controlnet_image],
                controlnet_guidance_scales=[controlnet_guidance_scale],
                controlnet_conditioning_modes=[int(controlnet_conditioning_mode)],
                width=controlnet_image.width,
                height=controlnet_image.height,
                guidance_scale=guidance_scale,
