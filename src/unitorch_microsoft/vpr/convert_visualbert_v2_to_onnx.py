...
class VisualBertV2ForQuery(nn.Module):
    ...
    def forward(
        ...
        query_input_ids = torch.cat([torch.tensor([101], dtype=torch.int64).reshape(1, -1), query_input_ids], dim=0)
        query_input_ids = torch.cat([query_input_ids, torch.tensor([102], dtype=torch.int64).reshape(1, -1)], dim=0)
...
model_file = "/disk/lixin/visualbert_v2.bin"
model = VisualBertV2ForQuery(config_path=cached_path("./configs/visualbert.argus.json"), weight_path=model_file)
model.eval()

# %%
query = torch.tensor([5929, 6007, 11912, 2308, 5929, 15695, 17764, 17447, 2015]).unsqueeze(-1)
...
# %%
tokens = tokenizer.tokenize("collectibles fine art sports photographs")
input_ids = tokenizer.convert_tokens_to_ids(tokens)
print(input_ids)
...
query = torch.tensor([8145, 7028, 2015, 2986, 2396, 2998, 7008]).unsqueeze(-1)
